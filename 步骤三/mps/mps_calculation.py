...
def calculate_mps(df, n, total_hours, min_productions):
    ...
    for i, min_production in enumerate(min_productions):
        ...
        if total_production < min_production:
            ...
            # 按比例分配增加的生产量
            while additional_production > 0 and remaining_hours > 0:
                for index, row in df.iterrows():
                    ...
                    if total_safety_stock > 0:
                        ...
                        additional_units = round_up_to_nearest_x(math.ceil(proportion * additional_production), min_production)
                    ...
                    print(f"Add {additional_units} units of {stock_code} to month {month} to meet minimum production requirement.")
