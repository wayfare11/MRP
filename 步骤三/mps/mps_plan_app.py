...
class MPSPlanApp(QMainWindow, MPSPlanUi):
    ...
    def run_operations(self):
        ...
        try:
            ...
            month1_min_production = float(self.month1MinProductionInput.text())
            month2_min_production = float(self.month2MinProductionInput.text())
            month3_min_production = float(self.month3MinProductionInput.text())
            month4_min_production = float(self.month4MinProductionInput.text())
        except ValueError:
            QMessageBox.warning(self, "警告", "请输入有效的工时值和月最低生产量（数字）")
            self.log_to_main_window("操作失败：无效的工时值或月最低生产量")
            return

        working_hours = [month1_hours, month2_hours, month3_hours, month4_hours]
        min_productions = [month1_min_production, month2_min_production, month3_min_production, month4_min_production]
        ...
        try:
            ...
            original_columns = df_sales_forecast.columns
            
            # 判断是否有重复的'存货编码'
            if df_sales_forecast['存货编码'].duplicated().any():
                print("存在重复的存货编码")
                
                # 找到列名中包含“订单”、“预测”和“合计”的列
                sum_columns = [col for col in df_sales_forecast.columns if any(keyword in col for keyword in ['订单', '预测', '合计'])]
                
                # 定义聚合操作，sum_columns进行求和，其余列保留第一个值
                agg_dict = {col: 'sum' for col in sum_columns}
                other_columns = [col for col in df_sales_forecast.columns if col not in sum_columns + ['存货编码']]
                for col in other_columns:
                    agg_dict[col] = 'first'
                
                # 按'存货编码'列进行分组并进行聚合
                df_sales_forecast = df_sales_forecast.groupby('存货编码', as_index=False).agg(agg_dict) 

                # 重新排列列名
                df_sales_forecast = df_sales_forecast[original_columns]      
            ...
            df_sales_forecast.loc[df_sales_forecast['标准工时'] == 0, '标准工时'] = 5
            ...
            for i, month in enumerate(months):
                ...
                self.log_to_main_window(f"Calculating MPS for month: {month}")
                mps_df, error_message = calculate_mps(df_sales_forecast, month, working_hours, min_productions)
