...
def round_up_to_nearest_50(x):
    """将数值向上取整到最近的50的倍数"""
    return max(0, ((x + 49) // 50) * 50)
...
def calculate_mps_for_item(a, b, c, d, e, safety_stock, n, get_next_month):
    ...
    # 获取求解结果并调整到50的倍数
    mps_n = round_up_to_nearest_50(pulp.value(P6) if pulp.value(P6) is not None else 0)
    mps_n1 = round_up_to_nearest_50(pulp.value(P7) if pulp.value(P7) is not None else 0)
    mps_n2 = round_up_to_nearest_50(pulp.value(P8) if pulp.value(P8) is not None else 0)
    mps_n3 = round_up_to_nearest_50(pulp.value(P9) if pulp.value(P9) is not None else 0)
